...
| Mode | Set | Behavior |
| --- | --- | --- |
| Managed (default) | nothing | Launches the detected browser headless on the account's profile, `~/.local/share/figma-reader/accounts/<name>/profile-<browser>` |
...
| `FIGMA_READER_CACHE` | `~/.cache/figma-reader` | Root of the snapshot cache (`.fig` files from web exports); each account keeps its own `accounts/<account>/` below it |
